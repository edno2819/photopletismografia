...
def detecta_face(frame):
    ...
    face_cascade_name = 'files/haarcascade_frontalface_default.xml'  # classificador
...
def detecta_olho(roi_gray, roi_color):
    ...
    eye_cascade_name = 'files/haarcascade_eye.xml'              # classificador
...
def gravar_video(nome):
    ...
    arquivo = "saves/testevideo_" + nome + ".avi"
...
def calcular_fc(raw_ppg, fs):
    ...
    figure_name = "saves/picos.png"
    figura.savefig(figure_name, dpi=600)
    ...
    axis[0, 2].plot(frequencia, amplitude)
    axis[0, 2].set_title("Sinal fft de amplitude em frequência")
    axis[0, 2].set_ylabel("Amplitude")
    axis[0, 2].set_xlabel("Frequência (Hz)")

    axis[1, 0].plot(df.index.array, df.duration)
    axis[1, 0].set_title("Sinal ECG")
    axis[1, 0].set_ylabel("Amplitude")
    axis[1, 0].set_xlabel("Tempo")

    axis[1, 1].imshow(LAST_FRAME)
    axis[1, 1].set_title("Imagem analisada")

    image = mpimg.imread(figure_name)
    axis[1, 2].imshow(image)
    axis[1, 2].axis('off')

    # Combine all the operations and display
    timestamp = datetime.timestamp(datetime.now())
    figure.savefig(f'saves/analise_completa_{timestamp}.png', dpi=600)
    plt.show()
